...
\begin{proof}[NOTDONE]
    ...
    % where \(v=(-1)^{p^s-1}\). does not divide nf^1tt, because p does not divide tt
    % So, p does not divide v.
    % Now, t =
    % ••
    % r
    % Ap+v
    % implies u(t - 1) = p(A - tp)
    % \Rightarrow v I i)
    % \Rightarrow p j (t - 1), because pdoes not divide v.
    % PP+v
    % i ?•
    % - i
    % :
...
\end{proof}
    ...
    % Corollary 10.8 The number of distinct Sylow p-subgroups divides the p-free part of
    % |G|.That is, if|G|= prm,where p does not divide m,then A (the number of distinct
% Sylow p-subgroups of G) divides m.
% Proof. Let G be a group with |G| = pTm (where p does not divide rri) and A is the
...
% conjugate of G. ; -
% So, A
% - [G: Afp ] is the index of the normalizer of P in G .
% Np contains P, because P is a subgroup.
% Since|/Vp|divides|G| = prm and is > |P| = pr,
% it follows that|/VP|= prm', where p does not divide m'.
% Sc, A = [G: Np] =
% \Rightarrow Am' = m.
% A divides m.
% Thus, not only is the number of distinct Sylow p-subgroups a number of the very
% special form 1 + kp, k 0, but it is also a divisor of m.
% Historical Note. Sylow (Ludyig Sylow, 1832-1918) stated and proved his theorems in
...
% proofs of Sylow’s second and third theorems and that of Theorem 10.5 via group
...
% Theorem 1Q.7 (Frobenius) Every p-group G has normal subgroups of every order
% dividing|G|, and their number is = 1 (mod p).
% Proof. Let G be a group with|G 1 = pr, where r > 1is a natural number.
% By Sylow's first theorem, G has subgroups of every order pk dividing|G| = pr.
% For any fixed k , l k r, let X be the set of k subgroups of order pk.
% Let G act on X by conjugation, (a, H ) -^ccHa' 1, a e G a n d W e l
% Every orbit H has a length which divides |G| = pr;
...
% that|*|= t + sp.
% By Sylov/s third theorem,|*| = l (mod p).
% Hence t = 1 (mod p).
...
% Cauchy's Theorem
% Theorem 10.8 Let G be a finite group and let p be a prime number dividing|C7|. Then G
...
% •
% l
% Since there are |G| choices fdr each of elements x1,x2,...,xp_1, we conclude:|*| =
% ICP"1-
% So, p divides ] X\, because p divides|C|and p > 2.
% A
...
% 1
% Is
% As pdivides|X |X<c r> J must contain elements besides (e, e, e,..., e).
